...
class TA_Features:
    ...
    unique_pandas_ta_indicators = {
            ...
            'kst': ta.kst, 'kurtosis': ta.kurtosis, 'decay': ta.decay, 
    ...
    pandas_ta_indicators = {
            ...
            'decay': ta.decay, 'log_return': ta.log_return, 
    ...
    # Wrapper functions to get all indicators
    @classmethod
    ...
    @classmethod
    def get_one_pandas_ta_indicator(cls, indicator, data):
        """
            Get a Pandas_TA indicator
            :param: data - DataFrame with columns: 'open' 'high' 'low' 'close' 'volume'
        """
        ...
        if indicator == 'massi':
            indicator_data = cls.pandas_ta_indicators[indicator](high=high, low=low)
        else:
            indicator_data = cls.pandas_ta_indicators[indicator](open_=open_,
                                                                 high=high,
                                                                 low=low,
                                                                 close=close,
                                                                 volume=volume)
    ...
    @classmethod
    def get_one_pantulipy_indicator(cls, indicator, data):
        """
            Get one Tulip Indicator
            :param: data - DataFrame with columns: 'open' 'high' 'low' 'close' 'volume'
        """

        if indicator not in pantulipy.core._DEFAULTLESS_INDICATORS:
            data = pd.concat([data, cls.pantulipy_indicators[indicator](data)], axis=1)
        return cls._remove_duplicate_columns(data)