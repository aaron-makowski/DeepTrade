...
class PlotlyTradingChart(BaseRenderer):
    ...
    https://plot.ly/python/figurewidget/           https://plot.ly/python/subplots/
    https://plot.ly/python/reference/#candlestick  https://plot.ly/python/#chart-events
    """
    def __init__(self, 
                 display: bool = True, 
                 height: int = None,
    ...
    def _create_figure(self, performance_keys):
        ...
        performance_keys = [
            col[col.find(':')+1:].replace('/','') if ':' in col else col for col in performance_keys
        ]
        ...
        fig.update_xaxes(title_standoff=7, title_font={'size': 12})

        self.fig = go.Figure(fig)#Widget(fig)
    ...
    def render(self, episode: int = None, 
        ...
        # Format Column names for
        performance.columns = [
            col[col.find(':')+1:].replace('/','') if ':' in col else col for col in performance.columns
        ]
        ...
        if self.save_as_json:
            self.fig.write_json('./examples/frontend/plotly_fig.json')
    ...
    def save(self):
        ...
        if self._save_format == 'html':
            self.fig.write_html(
                ...
                auto_open=self._auto_open_html)
        else:
